...
const clearCollection = async (collectionPath) => {
    ...
    await batch.commit();
    console.log(`🗑️  Cleared global collection: ${collectionPath}`);
...
const seedEconoSim = async (users) => {
    // 1. Clear previous global data
    await clearCollection('econosim_scenarios');
    await clearCollection('econosim_simulations');
    console.log("--- ✅ EconoSim global collections cleared. ---");
    
    // 2. Seed Scenarios (global collection)
    ...
    // 3. Create initial simulation state for each user in the top-level collection
    ...
    for (const user of usersToSeed) {
        ...
        batch.set(docRef, {
            uid: user.uid, // This is the critical field for security rules
            scenarioId,
            status: 'not-started',
            inputs: {
                interestRate: 0,
